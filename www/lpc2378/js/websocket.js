...
var WSN_CLIENT = (function () {
    ...
    function createTimeline() {
        chartAcc = new SmoothieChart({strokeStyle: 'rgb(125, 0, 0)', fillStyle: 'rgb(60, 0, 0)', lineWidth: 1, maxValue: 900, minValue: 300, millisPerLine: 250, verticalSections: 6 });
    ...
    return {
        linkLedCommand : function () {
            var data = JSON.stringify({id : 'SN01', type : 'COMMAND', to : 'LINK_LED_TOGGLE'});
            websocket.send(data);
        },
        connectLedCommand : function () {
            var data = JSON.stringify({id : 'SN01', type : 'COMMAND', to : 'CONNECT_LED_TOGGLE'});
            websocket.send(data);
        },
        sdLedCommand : function () {
            var data = JSON.stringify({id : 'SN01', type : 'COMMAND', to : 'SD_LED_TOGGLE'});
            websocket.send(data);
        },
